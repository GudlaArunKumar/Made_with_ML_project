...
def load_dict(path: str) -> Dict:
    """Load a dictionary from Json filepath
...
def collate_fn(batch: Dict[str, np.ndarray]) -> Dict[str, torch.Tensor]:  # pragma: no cover, air internal
    """Convert a batch of numpy arrays into torch tensors with appropiate padding

    Args:
        batch (Dict[str, np.ndarray]): nput batch as a dictionary of numpy arrays.
