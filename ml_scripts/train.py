...
import typer # library for CLI interface 
...
from ml_scripts import data, config, utils 
...
#initializing Typer CLI app interface 
app = typer.Typer()


# defining all required functions for training model 
def train_step(ds: Dataset, 
               batch_size: int, 
               model: nn.Module, 
               num_classes: int, 
    ...
    """ Train step function which trains and calculate training
      loss for each epoch

    Args:
        ds (Dataset): dataset to train in batches.
...
def eval_step(
    ...
    num_classes: int, 
    loss_fn: torch.nn.modules.loss._WeightedLoss
):  # pragma: no cover, tested via train workload
    """Evaluation step to calculate validation loss while training during each epoch

    Args:
        ds (Dataset): dataset to iterate in batches
        batch_size (int): size of each batch.
        model (nn.Module): model to train.
    ...
    Returns:
        Tuple[float, np.array, np.array]: cumulative validation loss, ground truth labels and predictions.
    """
    model.eval()
    loss = 0.0
    y_trues, y_preds = [], []
...
def train_loop_per_worker(config: dict) -> None:
    """Training loop that each worker node will execute during training
    ...
    utils.set_seeds()
    train_ds = r_train.get_dataset_shard("train")
    val_ds = r_train.get_dataset_shard("val")
    

    # loading the model
    llm = BertModel.from_pretrained("allenai/scibert_scivocab_uncased", return_dict=False)
    model = FineTunedLLM(llm=llm, dropout_p=dropout_p, embedding_dim=llm.config.hidden_size, num_classes=num_classes)
    model = r_train.torch.prepare_model(model) 
    ...
    num_workers = r_train.get_context().get_world_size()
    batch_size_per_worker = batch_size // num_workers 
    for epoch in range(num_epochs):
        # training step 
        train_loss = train_step(train_ds, batch_size_per_worker, model, num_classes, loss_fn, optimizer)
        val_loss, _, _ = eval_step(val_ds, batch_size_per_worker, model, num_classes, loss_fn)
        scheduler.step(val_loss) # scheduler checks for min validation loss 
        ...
        with tempfile.TemporaryDirectory() as dp:
            ...
            r_train.report(metrics, checkpoint=checkpoint)
            


# To initiate training of the LLM model
@app.command() # to run this script using typer cli 
def train_model(
    ...
    num_workers: Annotated[int, typer.Option(help="number of worker nodes to use for training.")] = 1,
    ...
    # setting up training loop config
    ...
    # setting up mlflow callback config
    ...
    run_config = RunConfig(callbacks=[mlflow_callback], checkpoint_config=checkpoint_config, 
                           storage_path=EFS_DIR, local_dir=EFS_DIR, failure_config=r_train.FailureConfig(max_failures=-1))
    ...
    # Dataset config
    options = ray.data.ExecutionOptions(preserve_order=True) # deterministic process for distributed data training
    ...
    train_ds = train_ds.materialize() # for caching purpose
    val_ds = val_ds.materialize()

    # Trainer to initialize the training with all setup configurations
...
if __name__ == "__main__":
    """
    initilizating the ray and typer app CLI where train model function can be 
    invoked via command line interface with required arguments
    """
