...
# script to perform behaviorual testing of the model for various inputs
...
def test_invariance(input_a, input_b, label, predictor):
    """test of InVariance via verb injection (changes should not affect outputs)."""
...
@pytest.mark.parametrize(
    ...
    [
        (
            "ML applied to text summarization.",
            "natural-language-processing",
        ),
        (
...
def test_directional(input, label, predictor):
    """Test of Directional expectations (changes with known outputs)."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "MLOps is the future in machine learning scope.",
            "mlops",
        ),
        (
